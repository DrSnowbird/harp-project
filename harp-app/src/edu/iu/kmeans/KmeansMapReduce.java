...
public class KmeansMapReduce extends Configured implements Tool {
  ...
  public static void main(String[] argv) throws Exception {
    int res = ToolRunner.run(new Configuration(), new KmeansMapReduce(), argv);
    System.exit(res);
  }
  ...
  public int run(String[] args) throws Exception {
    if (args.length < 10) {
      System.err
        .println("Usage: edu.iu.kmeans.KMeansMapReduce <num Of DataPoints> <num of Centroids> <vector size> "
  ...
  private void launch(int numOfDataPoints, int numCentroids, int vectorSize,
    ...
    if (generateData) {
      ...
      DataGen.generateData(numOfDataPoints, numCentroids, vectorSize,
        numPointFiles, configuration, fs, dataDir, cDir, localPointFilesDir,
        startJobID);
    }
    ...
    runKMeansAllReduce(numOfDataPoints, numCentroids, vectorSize,
      ...
      configuration, workDirPath, dataDir, cDir, outDir);
  ...
  private void runKMeansAllReduce(int numOfDataPoints, int numCentroids,
    ...
    do {
      ...
      Job kmeansJob = configureKMeansJob(numOfDataPoints, numCentroids,
        ...
        dataDir, cDir, outDir, jobIDCount, iterationPerJob);
      System.out.println("| Job#" + jobIDCount + " configure in "
      ...
      System.out.println("end Jod#"
        ...
        + new SimpleDateFormat("HH:mm:ss.SSS").format(Calendar.getInstance()
          .getTime()));
      System.out.println("| Job#" + jobIDCount + " Finished in "
        + (System.currentTimeMillis() - perJobSubmitTime) + " miliseconds |");
      // -----------------------------------------------------------------------
      if (!jobSuccess) {
        System.out.println("KMeans Job failed. Job Count:" + jobIDCount);
        jobRetryCount++;
        if (jobRetryCount == 3) {
          break;
        }
      } else {
        iterationCount = iterationCount + iterationPerJob;
    ...
    System.out.println("Number of jobs = " + jobIDCount
  ...
  private Job configureKMeansJob(int numOfDataPoints, int numCentroids,
    ...
    job.setMapperClass(NoSyncKMeansAllReduceMapper.class);
