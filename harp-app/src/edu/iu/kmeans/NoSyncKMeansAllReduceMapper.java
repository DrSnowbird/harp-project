...
public class NoSyncKMeansAllReduceMapper extends
  ...
  private void runKmeans(List<String> fileNames, int numCenPartitions,
    ...
    for (int i = 0; i < this.iteration; i++) {
      ...
      Set<Map<Integer, DoubleArray>> localCenDataMaps = doTasksReturnSet(
        pDataList, "centroids-calc",
        new NoSyncCenCalcTask(cPartitions, cParSize, rest, numCenPartitions,
          vectorSize, this.getResourcePool(), context), numThreads);
      /*
      Set<Map<Integer, DoubleArray>> localCenDataMaps = doThreadTasks(
        pDataList, "centroids-calc", new NoSyncCenCalcTask(cPartitions,
          cParSize, rest, numCenPartitions, vectorSize, this.getResourcePool(),
          context), numThreads);
       */
      ...
      doTasks(partitionIDs, "centroids-merge",
        new NoSyncCenMergeTask(new ArrayList<Map<Integer, DoubleArray>>(
          localCenDataMaps), cenDataMap), numThreads);
      // Release local centroids map
   ...
   * @return
   */
  static Map<Integer, DoubleArray> createCenDataMap(int cenParSize,
